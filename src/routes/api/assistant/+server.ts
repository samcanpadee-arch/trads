...
async function deleteVectorStore(apiKey: string, vectorStoreId: string) {
...
// Detect "exact spec" tokens in GENERAL answers
...
if (approvedStoreId && share && uploaded.length) {
  ...
  console.log(`[assistant] share=true files=${uploaded.length} approved=${approvedStoreId}`);
    ...
    if (uploaded.length) {
      ...
      for (const f of uploaded) await attachFileToVectorStore(API, tempVectorStoreId, f.id);
      await waitForIndexing(API, tempVectorStoreId);
    }

    // Library stores from registry JSON
    const libraryIds: string[] = [privateEnv.PRIVATE_ASSISTANT_APPROVED_STORE_ID].filter(Boolean) as string[];

    // If opted-in, attach uploaded files to all library stores
    console.log("[assistant] share=%s files=%d approved=%s", share, uploaded.length, privateEnv.PRIVATE_ASSISTANT_APPROVED_STORE_ID);
        if (share && uploaded.length && libraryIds.length) {
      for (const libId of libraryIds) {
        for (const f of uploaded) {
                  console.log("share-attach", { file: f.filename, id: f.id, to: libId });
try { await attachFileToVectorStore(API, libId, f.id); } catch {}
        }
      }
    }

    // Strict system rules (stronger citation requirement)
    const SYSTEM = `
You are a technical assistant for experienced Australian tradies.

RETRIEVAL & CITATIONS (MANDATORY):
- Prefer content retrieved from manuals/standards first.
...
- IF YOU STATE ANY NUMERIC SPECIFICATION (numbers with units), IT MUST BE IMMEDIATELY FOLLOWED BY A BRACKETED CITATION AS ABOVE.
...
`.trim();
    ...
    // SOFT NUDGE: if MANUAL but no page/clause-like pattern, append note
    ...
    // best-effort cleanup of the per-session store to avoid retention/cost
    try {
      if (tempVectorStoreId && privateEnv.OPENAI_API_KEY) {
        await deleteVectorStore(privateEnv.OPENAI_API_KEY, tempVectorStoreId);
      }
    } catch {}
  }
};
